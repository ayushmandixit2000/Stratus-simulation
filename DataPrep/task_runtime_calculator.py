...
def calculate_task_runtimes(file_patterns):
    """
    Calculate task runtimes from Google cluster data by finding the duration between
    submission (event_type=0) and completion (event_type=4) events. Include CPU and memory requests.
    
    Args:
        file_patterns: List of file pattern strings for task event files.
                       Example:
                           [
                               './task_events/task_events/part-00000-of-00500.csv.gz',
                               './task_events/task_events/part-00001-of-00500.csv.gz',
                               './task_events/task_events/part-00002-of-00500.csv.gz',
                               './task_events/task_events/part-00003-of-00500.csv.gz'
                           ]
        
    Returns:
        DataFrame with task details including job_ID, task_index, CPU/memory requests, 
        timestamps, and runtime in seconds,
        and the maximum timestamp across all files.
    """
    import glob
    import pandas as pd
    ...
    for pattern in file_patterns:
        for filename in glob.glob(pattern):
            ...
            # Remove rows with timestamp 0 as they are invalid due to occurring before the trace
            df = df[df["timestamp"] != 0]
    ...
    # Get submit events (event_type == 0) and finish events (event_type == 4)
    submits = combined_events[combined_events['event_type'] == 0].drop_duplicates('task_id')
    finishes = combined_events[combined_events['event_type'] == 4].drop_duplicates('task_id')
    
    # Merge submit and finish events on task_id
    task_runtimes = pd.merge(
        submits[['task_id', 'timestamp', 'cpu_request', 'memory_request']],
        finishes[['task_id', 'timestamp']],
        on='task_id',

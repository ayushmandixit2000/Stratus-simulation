...
#         # Update instance's runtime and timestamp correctly
#         task_end_time = task['timestamp'] + task['runtime']
#         current_instance_end_time = instance['timestamp'] + instance['runtime']
#         new_end_time = max(task_end_time, current_instance_end_time)
        
#         # Instance's start time remains the earliest timestamp
#         new_start_time = min(instance['timestamp'], task['timestamp'])
#         new_runtime = new_end_time - new_start_time
        
#         self.instance_bins.at[instance_idx, 'timestamp'] = new_start_time
#         self.instance_bins.at[instance_idx, 'runtime'] = new_runtime
        
#         # Calculate additional runtime to charge
#         additional_runtime = new_end_time - current_instance_end_time
...
#                 # Randomize for accuracy
#                 randomized_instance_types = self.available_instance_types.sample(frac=1).reset_index(drop=True)
#                 for _, instance_type in randomized_instance_types.iterrows():
#                     if (
